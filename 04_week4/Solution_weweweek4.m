...
for ii = 2:Nx-1
    for jj = 2:Ny-1
        ...
        A(node11, node11) = -2*kappa*(1/dx^2 + 1/dy^2);   % Define the the A-matrix for the internal nodes
        A(node11, node21) = kappa/dx^2;
        A(node11, node01) = kappa/dx^2;
        A(node11, node12) = kappa/dx^2;
        A(node11, node10) = kappa/dx^2;
    end
end
...
rho0 = c0 * exp(-((X - x0).^2 + (Y - y0).^2) / (2 * a^2));
rho0 = reshape(rho0, nn, 1);
%% Time integration
...
for kk = 2:NT
    % Calculate the Laplacian of rho at the current and next time steps
    laplacian_rho_current = A * rho(:, kk-1);
    laplacian_rho_next = A * rho(:, kk);
    
    % Time integration step (Crank-Nicolson)
    rho(:, kk) = rho(:, kk-1) + 0.5 * dt * (laplacian_rho_current + laplacian_rho_next);
end
